...
# options passed:  ../src/one/x.t.c -mavx512vl -mtune=generic -march=x86-64
# -O0 -fverbose-asm
...
# ../src/one/x.t.c:10: 	unsigned long long int acc = 0;
	movq	$0, 256(%rsp)	#, acc
# ../src/one/x.t.c:12: 	int i=0;
	movl	$0, 248(%rsp)	#, i
# ../src/one/x.t.c:14: 		temp[0]=i;
	movl	248(%rsp), %eax	# i, tmp91
	cltq
	movq	%rax, -56(%rsp)	# _1, temp
# ../src/one/x.t.c:15: 		_mm512_stream_si512(&mem[i], temp);
...
# /usr/local/gcc-9.3/lib/gcc/x86_64-linux-gnu/9.3.0/include/emmintrin.h:1511: }
	nop	
# ../src/one/x.t.c:18: 	for(int i=0; i < ARRAY_SIZE; i++)
	movl	$0, 252(%rsp)	#, i
# ../src/one/x.t.c:18: 	for(int i=0; i < ARRAY_SIZE; i++)
	jmp	.L2	#
.L4:
# ../src/one/x.t.c:20: 		local = _mm512_load_si512(&mem[i]);
...
# /usr/local/gcc-9.3/lib/gcc/x86_64-linux-gnu/9.3.0/include/avx512fintrin.h:547:   return *(__m512i *) __P;
	...
	vmovdqa64	(%rax), %zmm0	# MEM[(__m512i * {ref-all})__P_24], D.24072
# ../src/one/x.t.c:20: 		local = _mm512_load_si512(&mem[i]);
	vmovdqa64	%zmm0, 8(%rsp)	# D.24072, local
# ../src/one/x.t.c:21: 		acc += *((uint64_t*)&local);
...
# /usr/local/gcc-9.3/lib/gcc/x86_64-linux-gnu/9.3.0/include/emmintrin.h:1511: }
	nop	
# ../src/one/x.t.c:25: 	return (int)acc;
