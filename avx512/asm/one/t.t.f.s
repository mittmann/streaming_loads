...
# options passed:  ../src/one/t.t.f.c -mavx512vl -mtune=generic
...
# ../src/one/t.t.f.c:10: 	unsigned long long int acc = 0;
	movq	$0, 256(%rsp)	#, acc
# ../src/one/t.t.f.c:12: 	for(int i=0; i<ARRAY_SIZE; i++)
	movl	$0, 252(%rsp)	#, i
# ../src/one/t.t.f.c:12: 	for(int i=0; i<ARRAY_SIZE; i++)
	jmp	.L2	#
.L3:
# ../src/one/t.t.f.c:15: 		temp[0]=i;
	movl	252(%rsp), %eax	# i, tmp91
	cltq
	movq	%rax, -56(%rsp)	# _1, temp
# ../src/one/t.t.f.c:16: 		_mm512_store_si512(&mem[i], temp);
	vmovdqa64	-56(%rsp), %zmm0	# temp, temp.0_2
# ../src/one/t.t.f.c:16: 		_mm512_store_si512(&mem[i], temp);
	movl	252(%rsp), %eax	# i, tmp93
	cltq
	salq	$6, %rax	#, tmp94
...
# /usr/local/gcc-9.3/lib/gcc/x86_64-linux-gnu/9.3.0/include/avx512fintrin.h:580:   *(__m512i *) __P = __A;
	...
	vmovdqa64	%zmm0, (%rax)	# tmp96, MEM[(__m512i * {ref-all})__P_25]
...
# /usr/local/gcc-9.3/lib/gcc/x86_64-linux-gnu/9.3.0/include/emmintrin.h:1511: }
	nop	
# ../src/one/t.t.f.c:23: 	for(int i=0; i < ARRAY_SIZE; i++)
	movl	$0, 248(%rsp)	#, i
# ../src/one/t.t.f.c:23: 	for(int i=0; i < ARRAY_SIZE; i++)
	jmp	.L4	#
.L6:
# ../src/one/t.t.f.c:25: 		local = _mm512_load_si512(&mem[i]);
	movl	248(%rsp), %eax	# i, tmp98
	cltq
	salq	$6, %rax	#, tmp99
	addq	$mem.24050, %rax	#, _4
	movq	%rax, 128(%rsp)	# _4, __P
# /usr/local/gcc-9.3/lib/gcc/x86_64-linux-gnu/9.3.0/include/avx512fintrin.h:547:   return *(__m512i *) __P;
	movq	128(%rsp), %rax	# __P, tmp100
	vmovdqa64	(%rax), %zmm0	# MEM[(__m512i * {ref-all})__P_27], D.24076
# ../src/one/t.t.f.c:25: 		local = _mm512_load_si512(&mem[i]);
	vmovdqa64	%zmm0, 8(%rsp)	# D.24076, local
# ../src/one/t.t.f.c:26: 		acc += *((uint64_t*)&local);
...
# /usr/local/gcc-9.3/lib/gcc/x86_64-linux-gnu/9.3.0/include/emmintrin.h:1511: }
	nop	
# ../src/one/t.t.f.c:30: 	return (int)acc;
