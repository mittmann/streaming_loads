...
# options passed:  ../src/rep/n.x.c -mavx512vl -mtune=generic -march=x86-64
# -O0 -fverbose-asm
...
# ../src/rep/n.x.c:10: 	unsigned long long int acc = 0;
	movq	$0, 256(%rsp)	#, acc
# ../src/rep/n.x.c:12:     for(int j=0; j<REP; j++){ 	
	movl	$0, 252(%rsp)	#, j
# ../src/rep/n.x.c:12:     for(int j=0; j<REP; j++){ 	
	jmp	.L2	#
.L6:
# ../src/rep/n.x.c:13: 	for(int i=0; i<ARRAY_SIZE; i++)
	movl	$0, 248(%rsp)	#, i
# ../src/rep/n.x.c:13: 	for(int i=0; i<ARRAY_SIZE; i++)
	jmp	.L3	#
.L4:
# ../src/rep/n.x.c:16: 		temp[0]=i;
	movl	248(%rsp), %eax	# i, tmp91
	cltq
	movq	%rax, -56(%rsp)	# _1, temp
# ../src/rep/n.x.c:17: 		_mm512_stream_si512(&mem[i], temp);
...
# /usr/local/gcc-9.3/lib/gcc/x86_64-linux-gnu/9.3.0/include/emmintrin.h:1511: }
	nop	
# ../src/rep/n.x.c:21: 	int i = 0;
	movl	$0, 244(%rsp)	#, i
# ../src/rep/n.x.c:23: 		local = _mm512_stream_load_si512(&mem[i]);
...
# ../src/rep/n.x.c:24: 		acc += *((uint64_t*)&local);
...
# ../src/rep/n.x.c:12:     for(int j=0; j<REP; j++){ 	
	cmpl	$999, 252(%rsp)	#, j
	jle	.L6	#,
# ../src/rep/n.x.c:28: 	return (int)acc;
