...
echo "\"prefetch\",\"size\",\"memtyp\",\"temp\",\"app\",\"displaysize\",\"cycles\",\"l3m\",\"l3l\""
		...
		for memtyp in wb wc; do
			for temp in t nt; do
				...
				for microsize in 524288 262144 131072 65536 32768 16384 8192 4096; do
					...
					sleep 5
					displaysize=`nice --20  perf stat -e LLC-load-misses,LLC-loads,cycles $microbenchdir/$bin $reps $microsize 2>/tmp/perfout | grep Memory | cut -f2 -d:`
					...
					echo $pref,$size,$memtyp,$temp,$bin,$displaysize,$cyc,$l3m,$l3l
				done
			done
				kill -PIPE $pid
			done
