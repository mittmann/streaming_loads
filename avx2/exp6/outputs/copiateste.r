library(dplyr);
#library(ggplot2);
library(tidyr);
library(outliers);
args = commandArgs(trailingOnly=TRUE)
df <- read.csv(args[1]);

k <- df %>% select(sizea,sizeb,temp,cyca,l3ma,l3ra,cycb,l3mb,l3rb) %>% 
 group_by(sizea,sizeb,temp) %>% summarise(mcyca=n(), mcycb=mean(cycb))  %>%  as.data.frame()  #%>% summarise(mean_cyc=mean(cycles), mean_l3m=mean(l3m), mean_l3l=mean(l3l), sd_c=sd(cycles), sd_m=sd(l3m),sd_l=sd(l3l))

k
quit
#head(k)
#k <- gather(k, cyca,l3ma,l3ra,cycb,l3rb,l3mb) #%>% as.data.frame()
#k <- subset(k, select = c(-cyca,-l3ra, -l3ma, -cycb, -l3rb, -l3mb))
k <- gather(k, key="key", value="value", -sizea, -sizeb, -temp) %>% as.data.frame()


lista <- k %>% group_by(temp, key, add = TRUE) %>% group_split(value)
#lista
#lista
for (t in lista) {
	#print (t)
    a <- spread(t, sizeb, value)
	#print(a)
	a <- a %>% mutate_if(is.factor, as.character)
	a <- a %>% mutate_if(is.integer, as.character)
	counter = a$key[1]
	temp = a$temp[1] 
	a <- select (a, -c(temp, key, sizea))
	write.table(a, paste0("exportables/",counter,temp, sep=""), row.names=FALSE, col.names=FALSE, sep=" ")
}

#lista

quit()

      write.csv(k, args[2]) 
