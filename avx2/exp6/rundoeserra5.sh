...
repts=( 14928740 1082620 128062 63822 31906 15567 9691 5722 1485 993 744 664 715 616 271 124 60 30 15 10 )
echo "" > log
...
do
    if [ "$f1" != "\"name\"" ]; then
		...
		while [ $ret -ne 0 ]; do
			sleep 1
			{ ./a.out $size1 $reps1 $mem1 $temp1 $size2 $reps2 $mem2 $temp2; } > tmpout 2>> log
			ret=$?
		done
...
done < doe5.csv #>> data/MM_output.csv
