...
# options passed:  ../src/one/x.t.c -mavx2 -mtune=generic -march=x86-64 -O0
# -fverbose-asm
...
# ../src/one/x.t.c:15: 		_mm256_stream_si256(&mem[i], temp);
...
# ../src/one/x.t.c:18: 	for(int i=0; i < ARRAY_SIZE; i++)
	jmp	.L2	#
.L4:
# ../src/one/x.t.c:20: 		local = _mm256_load_si256(&mem[i]);
	movl	60(%rsp), %eax	# i, tmp98
	cltq
	salq	$5, %rax	#, tmp99
...
# /usr/local/gcc-9.3/lib/gcc/x86_64-linux-gnu/9.3.0/include/avxintrin.h:910:   return *__P;
	...
	vmovdqa	(%rax), %ymm0	# *__P_24, D.24072
# ../src/one/x.t.c:20: 		local = _mm256_load_si256(&mem[i]);
